...
def test_user_profile_api_get_success(user_account, api_client):
    ...
    response = client.get(reverse("app-v1-accounts:user-profile"))
...
def test_user_profile_api_update_success(user_account, api_client):
    ...
    new_first_name = "Jane"
    user = user_account(
        email=email, first_name=old_first_name, last_name=old_last_name
    )
    client = api_client(auth_user=user)

    response = client.put(
        reverse("app-v1-accounts:user-profile"), {"first_name": new_first_name}
    )
