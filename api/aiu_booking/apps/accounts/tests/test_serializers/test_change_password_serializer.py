...
def test_validate_old_password_success(
    ...
    mocked_check_password.assert_called_once_with(
        change_password_serializer_request.user, OLD_PASSWORD
    )
...
def test_validate_new_password_success(
    ...
    mocked_validate_password.assert_called_once_with(
        NEW_PASSWORD, user=change_password_serializer_request.user
    )
...
def test_validate_new_password_failure(
    ...
    mocked_validate_password = mocker.patch.object(
        ...
        "validate_password",
        side_effect=[
            InvalidPasswordError("test_validate_new_password_failure")
        ],
    )
    ...
    mocked_validate_password.assert_called_once_with(
        OLD_PASSWORD, user=change_password_serializer_request.user
    )
...
def test_save(user_account, mocker):
    ...
    mocker.patch.object(
        serializer, "validate_old_password", return_value=OLD_PASSWORD
    )
    mocker.patch.object(
        serializer, "validate_new_password", return_value=NEW_PASSWORD
    )
    mocked_change_password = mocker.patch.object(
        serializer.password_service, "change_password"
    )
